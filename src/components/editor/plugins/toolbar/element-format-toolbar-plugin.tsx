  ...
  return (
    ...
    >
      ...
      {Object.entries(ELEMENT_FORMAT_OPTIONS).map(([value, option]) => (
        <ToggleGroupItem key={value} value={value} variant={'outline'} aria-label={option.name} className="size-8 p-0">
          {option.icon}
        </ToggleGroupItem>
      ))}
      ...
      {/* Indentation toggles */}
      <ToggleGroupItem value="outdent" aria-label="Outdent" variant={'outline'} className="size-8 p-0">
        <IndentDecreaseIcon className="size-4" />
      </ToggleGroupItem>

      <ToggleGroupItem value="indent" variant={'outline'} aria-label="Indent" className="size-8 p-0">
